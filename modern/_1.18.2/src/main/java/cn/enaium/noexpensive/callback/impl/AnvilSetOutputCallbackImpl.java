    ...
    public void interact(ItemStack output, int levelCost, boolean canTake) {
        if (!output.equals(ItemStack.EMPTY)) {
            if (output.getNbt() != null) {
                ...
                nbtCompound.getList(ItemStack.LORE_KEY, NbtElement.LIST_TYPE).add(NbtString.of(Text.Serializer.toJson(new TranslatableText("container.repair.cost", levelCost).styled(style -> style.withBold(true).withColor(canTake ? Formatting.GREEN : Formatting.RED)))));
            }
        }
    }
}
