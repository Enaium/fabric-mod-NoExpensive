    ...
    override fun execute(commandSource: CommandSource, args: Array<String>) {
        ...
        if (args[0].equals("compatibility", ignoreCase = true)) {
            if (args.size == 1) {
                        ...
                        val enchantment = LiteralText(key).setStyle(
                            Style().setHoverEvent(
                                HoverEvent(
                                    ...
                                    TranslatableText(Enchantment.getByName(key)?.translationKey ?: continue)
                                )
                                    ...
                                    LiteralText(s).setStyle(
                                        Style().setHoverEvent(
                                            HoverEvent(
                                                HoverEvent.Action.SHOW_TEXT,
                                                TranslatableText(
                                                    Enchantment.getByName(key)?.translationKey ?: continue
                                                )
                                            )
                                        ).setFormatting(Formatting.AQUA)
                                    )
                ...
                val compatibility = Config.model.compatibility
                val enchantment1 = args[2].takeIf { Enchantment.getByName(args[2]) != null } ?: let {
                    commandSource.sendMessage(TranslatableText("command.compatibility.notFound", args[2]))
                    return
                }
                val enchantment2 = args[2].takeIf { Enchantment.getByName(args[3]) != null } ?: let {
    ...
    override fun getAutoCompleteHints(
        ...
        if (strings.size == 3 || strings.size == 4) {
            if (strings[0].equals("compatibility", ignoreCase = true)) {
                return Enchantment.getSet().map { it.toString() }
            }
        }
        return emptyList()
    }
