    ...
    public void interact(ItemStack output, int levelCost, boolean canTake) {
        if (!output.equals(ItemStack.EMPTY)) {
            if (output.getTag() != null) {
                ...
                compoundTag.getList("Lore", 9).add(new StringTag(Text.Serializer.toJson(new TranslatableText("container.repair.cost", levelCost).styled(style -> style.setBold(true).setColor(canTake ? Formatting.GREEN : Formatting.RED)))));
            }
        }
    }
}
